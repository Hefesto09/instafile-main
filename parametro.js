...
document.getElementById("form").addEventListener("submit", (e) => {
  ...
  if (files.length > 0) {
    ...
    alert("Por favor, seleccione uno o más archivos primero.");
  }
});

// Función para manejar el archivo seleccionado en el área de arrastre
...
function uploadFile(carpetaNombre, file) {
  ...
  xhr.onload = function () {
    if (xhr.status === 200) {
      showSuccessMessage(`Archivo ${file.name} subido con éxito`);
      actualizarListaArchivos(carpetaNombre);
    } else {
      showErrorMessage(`Error al subir el archivo ${file.name}`);
    }
  };
...
// Agregar eventos para la zona de arrastre
